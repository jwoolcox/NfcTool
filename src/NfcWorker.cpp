...
void NfcWorker::prepareToEmulateEcho() {
	...
	char validId[] = { 0x4E, 0x01, 0x02, 0x05 }; //BY changing this value we change the id (What the door uses as ID)
...
void NfcWorker::prepareToWriteNdefSpTag(const QVariant &the_sp_uri,
	...
	emit message(
			QString("Writing SP URI: '%1' Text: '%2'").arg(sp_uri).arg(
					sp_text));
...
void NfcWorker::prepIso7816CardTest(const QVariant &aid, bool select_only,
		const QVariant &hex_cla, const QVariant &hex_ins,
		const QVariant &hexp1p2, const QVariant &hex_lc,
...
void NfcWorker::doIso7816Test(const QVariant &aid, bool select_only,
	...
	if (rc != NFC_RESULT_SUCCESS) {
		...
		if (rc == NFC_RESULT_SE_NOT_PRESENT) {
			emit message(
					QString("......or SIM does not include secure element"));
...
int NfcWorker::exchangeApdu(uchar_t* the_apdu, int apdu_size,
	...
	int rc = nfc_tag_transceive(target, TAG_TYPE_ISO_14443_4, the_apdu,
			apdu_size, response, MAX_RESPONSE_SIZE, &rlength);
	if (rc == NFC_RESULT_SUCCESS) {
		QByteArray responseData = QByteArray::fromRawData(
...
void NfcWorker::handleIso15693TagEvent(bps_event_t *event) {
	uint16_t code = bps_event_get_code(event);
	qDebug()
			<< "XXXX NfcWorker::handleIso15693TagEvent - processing event code "
			<< code;
...
void NfcWorker::handleGvbEvent(bps_event_t *event) {
	uint16_t code = bps_event_get_code(event);
	qDebug() << "XXXX NfcWorker::handleGvbEvent - processing event code "
			<< code;
				...
				for (int i = 0; i < TRANSACTION_NUMBER; i++) {
					...
					if (rc == NFC_RESULT_SUCCESS) {
						...
						if (counter > 0) {
							...
							emit message(
									QString("Transaction number: %1").arg(
											counter));
							emit message(
									QString("Date and time: %1 %2:%3").arg(
											epochDate.addDays(days).toString(
													"d MMM yyyy")).arg(hour, 2,
											10, QChar('0')).arg(mins, 2, 10,
											QChar('0')));
							...
							emit message(
									QString("Transport Network: %1").arg(
											locText));
							...
							emit message(
									QString("Transaction type: %1").arg(
											typeText));
						}
					} else {
						emit message(
								QString("ERROR transmitting APDU:%1").arg(rc));
						emit message(Utilities::getOperationResultName(rc));
					}
				}
			} else {
...
void NfcWorker::handleReadTagDetailsEvent(bps_event_t *event) {
	uint16_t code = bps_event_get_code(event);
	qDebug()
			<< "XXXX NfcWorker::handleReadTagDetailsEvent - processing event code "
			<< code;
	...
	if (NFC_SNEP_CONNECTION_EVENT == code) {
		...
		QString vCard =
				QString("BEGIN:VCARD\n").append("VERSION:3.0\n").append("N:").append(
						...
						"FN:").append(_first_name).append(" ").append(
						_last_name).append("\n").append("ADR;TYPE=WORK:").append(
						_address).append("\n").append("TEL;TYPE=CELL:").append(
...
void NfcWorker::handleEmulateNfcEvent(bps_event_t *event) {
	...
	qDebug()
			<< "XXXX NfcWorker::handleEmulateNfcEvent - processing event code: "
			<< code;
...
void NfcWorker::handleEmulateEchoEvent(bps_event_t *event) {
	...
	qDebug()
			<< "XXXX NfcWorker::handleEmulateEchoEvent - processing event code: "
			<< code;
...
void NfcWorker::displayTagInformation(nfc_target_t* target,
	...
	struct TagPropertyMap {
		...
		const char *name;
			...
			TAG_PROP_ISO_14443_4_TYPE, "ISO_14443_4 TYPE" }, {
			TAG_PROP_NDEF_TYPE, "NDEF TYPE" }, { TAG_PROP_NDEF_LOCKED,
			"NDEF LOCKED" }, { TAG_PROP_NDEF_LOCKABLE, "NDEF LOCKABLE" }, {
			TAG_PROP_NDEF_FREE_SPACE, "NDEF FREE SPACE" }, {
			...
			TAG_PROP_AFI_SUPPORTED, "AFI SUPPORTED" }, { TAG_PROP_AFI_LOCKED,
			"AFI LOCKED" }, { TAG_PROP_DSFID_SUPPORTED, "DSFID SUPPORTED" }, {
			TAG_PROP_DSFID_LOCKED, "DSFID LOCKED" }, { TAG_PROP_AFI, "AFI" }, {
			TAG_PROP_DSFID, "DSFID" }, { TAG_PROP_SECTOR_SIZE, "SECTOR SIZE" },
			{ TAG_PROP_SECTOR_NUMBER, "SECTOR NUMBER" }, { TAG_PROP_KOVIO_TYPE,
					"KOVIO TYPE" } };
	...
	struct TagVariantMap {
		...
		const char *name;
			...
			TAGVARIANT_DESFIRE_EV1_8K, "DESFIRE EV1 8K" }, {
			TAGVARIANT_TI_TAGIT, "TI TAGIT" }, { TAGVARIANT_ST_LRI_512,
			"ST LRI 512" }, { TAGVARIANT_ST_LRI_2K, "ST LRI 2K" }, {
	...
	for (uint_t i = 0;
			i < (sizeof(tagPropertyMap) / sizeof(struct TagPropertyMap)); i++) {
		qDebug() << "XXXX nfc_get_tag_property:" << tagPropertyMap[i].name;
		nfc_result_t rc = nfc_get_tag_property(target,
				tagPropertyMap[i].property, &buffer[0], sizeof buffer);
		if (rc == NFC_RESULT_SUCCESS) {
			if (strlen(&buffer[0]) > 0) {
				if (strcmp(tagPropertyMap[i].name, "NDEF TYPE") != 0) {
					qDebug() << "XXXX" << tagPropertyMap[i].name << buffer
							<< endl;
					emit message(
			...
			qDebug() << "XXXX nfc_get_tag_property() for"
					<< tagPropertyMap[i].name << "returned" << rc << endl;
		}
	}
	...
	if (rc == NFC_RESULT_SUCCESS) {
		...
		emit message(
				QString("Tag id: %1").arg(QString::fromAscii(uId.toHex())));
	} else {
	...
	if (rc == NFC_RESULT_SUCCESS) {
		for (uint_t i = 0;
				i < (sizeof(tagVariantMap) / sizeof(struct TagVariantMap));
				i++) {
			if (variant == tagVariantMap[i].variant) {
				qDebug() << "XXXX Tag variant" << tagVariantMap[i].name << endl;
				emit message(
						QString("Tag variant: %1").arg(tagVariantMap[i].name));
			}
		}
	} else {
