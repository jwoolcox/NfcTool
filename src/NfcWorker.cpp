...
unsigned char FILTER_APDU_1[] = { 0x00, 0xa4, 0x04, 0x00, 0x0c, 0xa0, 0x00,
		0x00, 0x00, 0x63, 0x50, 0x4b, 0x43, 0x53, 0x2d, 0x31, 0x35, 0x00 };
...
NfcWorker::NfcWorker(QObject *parent) :
		QObject(parent), BPS_EVENT_TIMEOUT(3000), _failedToInitialize(false), _timeToDie(
				false), _taskToPerform(NONE_SET), _navigatorExitReceived(false), _emulateNdefMessage(
				0) {
...
//hah hah ha ha hah door
void NfcWorker::getDefaultID(char *identbuff) {
	netstatus_interface_details_t *details = NULL;

	netstatus_get_interface_details(NULL, &details);

	const char* ifid = netstatus_interface_get_link_address(details);
...
/**
 * Some one had the BRILLIANT idea to base the identifier on the MAC Address\
 ...
 */
char dataStr[] = "Testing_data"; //affects nothing
...
if (rc == NFC_RESULT_SUCCESS) {
	...
	emit message(identifier_size);
	emit message(QString(applicationData));
	emit message(applicationData_size);
	emit message(identifier[0]);
	emit message(identifier[1]);
	emit message(identifier[2]);
	emit message(identifier[3]);
...
emit message(
		QString("Writing SP URI: '%1' Text: '%2'").arg(sp_uri).arg(sp_text));
...
emit message(
		QString("Writing Domain: '%1' Type: '%2' Payload: '%3'").arg(domain).arg(
				type).arg(payload));
...
qDebug() << "XXXX First Name: " << first_name << ", Last Name: " << last_name
		<< ", Address: " << address << ", Email: " << email << ", Mobile: "
		<< mobile;
...
void NfcWorker::prepIso7816CardTest(const QVariant &aid, bool select_only,
	const QVariant &hex_cla, const QVariant &hex_ins, const QVariant &hexp1p2,
	const QVariant &hex_lc, const QVariant &hex_command,
...
void NfcWorker::doIso7816Test(const QVariant &aid, bool select_only,
	const QVariant &hex_cla, const QVariant &hex_ins, const QVariant &hexp1p2,
	const QVariant &hex_lc, const QVariant &hex_command,
	const QVariant &hex_le) {
...
rc = nfc_se_session_open_logical_channel(hSESession, the_aid, aid_size,
...
if (!select_only) {
	...
	rc = nfc_se_channel_transmit_apdu(seChannel, the_apdu, apdu_size,
			&exchangeResponseLen);
	if (rc != NFC_RESULT_SUCCESS) {
		qDebug() << QString("XXXX ERROR transmitting APDU:%1").arg(rc);
		emit message(
				QString("ERROR transmitting APDU:%1:%2").arg(rc).arg(
						Utilities::getOperationResultName(rc)));
	}
	...
	if (exchangeResponseLen >= 2) {
		CHECK(
				nfc_se_channel_get_transmit_data(seChannel, &result[0], &nReceiveAPDUBufferSize));
		emit message(
				QString("got response APDU. Len=%1").arg(
						nReceiveAPDUBufferSize));
		QByteArray responseData = QByteArray::fromRawData(
				reinterpret_cast<const char *>(result), nReceiveAPDUBufferSize);
...
if (NFC_TAG_READWRITE_EVENT == code) {
	qDebug()
			<< "XXXX NfcWorker::exchangeApduWithCard - Target Read Write Event";

	emit message(QString("Card detected"));

	rc = nfc_get_nfc_event(event, &nfcEvent);
	if (rc != NFC_RESULT_SUCCESS) {
		QString msg = QString("Error: NFC result %1:%2").arg(rc).arg(
				Utilities::getOperationResultName(rc));
	...
	qDebug()
			<< "XXXX NfcWorker::exchangeApduWithCard - NFC BPS event that we didn't register for: "
			<< code;
}
...
int NfcWorker::miscCommand(uchar_t* the_command, int cmd_size,
	nfc_target_t* target) {
...
int NfcWorker::exchangeApdu(uchar_t* the_apdu, int apdu_size,
	nfc_target_t* target) {
QByteArray requestData = QByteArray::fromRawData(
		reinterpret_cast<const char *>(the_apdu), apdu_size);
...
int rc = nfc_tag_transceive(target, TAG_TYPE_ISO_14443_4, the_apdu, apdu_size,
		response, MAX_RESPONSE_SIZE, &rlength);
if (rc == NFC_RESULT_SUCCESS) {
	QByteArray responseData = QByteArray::fromRawData(
			reinterpret_cast<const char *>(response), rlength);
	QString responseAsHex = QString::fromAscii(responseData.toHex());
...
CHECK(
		nfc_create_ndef_message_from_bytes(reinterpret_cast<const uchar_t *>(data.data()), data.length(), &ndefMessage));
...
qDebug() << "XXXX NfcWorker::handleNfcReadNdefTagEvent - processing event code "
		<< code;
...
if (NFC_TAG_READWRITE_EVENT == code) {
	...
	for (unsigned int ndefMsgIndex = 0; ndefMsgIndex < ndefMsgCount;
			++ndefMsgIndex) {
...
uint16_t code = bps_event_get_code(event);
qDebug() << "XXXX NfcWorker::handleNfcWriteSpTagEvent - processing event code "
		<< code;
...
if (NFC_TAG_READWRITE_EVENT == code) {
	...
	displayTagInformation(target, false);
	qDebug()
			<< "XXXX NfcWorker::handleWriteTextTagEvent - Preparing to write Text: "
			<< _ndefText;
...
uint16_t code = bps_event_get_code(event);
qDebug() << "XXXX NfcWorker::handleWriteUriTagEvent - processing event code "
		<< code;
...
uint16_t code = bps_event_get_code(event);
qDebug() << "XXXX NfcWorker::handleIso15693TagEvent - processing event code "
		<< code;
...
if (NFC_TAG_READWRITE_EVENT == code) {
	qDebug()
			<< "XXXX NfcWorker::handleIso15693TagEvent - Target Read Write Event";
	...
	if (_taskToPerform == WRITE_ISO15693) {
		...
		qDebug()
				<< "XXXX NfcWorker::handleIso15693TagEvent - Preparing to write to ISO15693 tag";
...
//			uint8_t test[4] = { 1, 2, 3, 4 };
		...
		int rc = nfc_tag_transceive(target, TAG_TYPE_ISO_15693_3, command,
				clength, response, MAX_RESPONSE_SIZE, &rlength);
	...
	qDebug()
			<< "XXXX NfcWorker::handleIso15693TagEvent - NFC BPS event that we didn't register for: "
			<< code;
}
}
...
if (NFC_TAG_READWRITE_EVENT == code) {
	...
	if (_taskToPerform == READ_GVB) {
		qDebug()
				<< "XXXX NfcWorker::handleGvbEvent - Preparing to read GVB tag";
		...
		if ((rc == NFC_RESULT_SUCCESS) && (variant == TAGVARIANT_MIFARE_UL)) {
			...
			for (int i = 0; i < TRANSACTION_NUMBER; i++) {
				rc = nfc_tag_transceive(target, TAG_TYPE_ISO_14443_3,
						commands[i], sizeof(commands[i]), transactions[i].pages,
						sizeof(transactions[i].pages), &rlength);

				if (rc == NFC_RESULT_SUCCESS) {

					QByteArray responseData =
							QByteArray::fromRawData(
									reinterpret_cast<const char *>(transactions[i].pages),
									rlength);
					QString responseAsHex = QString::fromAscii(
							responseData.toHex());

					qDebug()
							<< "XXXX NfcWorker::handleGvbEvent - APDU response:"
							<< responseAsHex;

					/// painful since ints are different endian :-(

					unsigned int counter =
							EXTRACT_CNTR(ENDIAN_RET32(transactions[i].transdata0));
					unsigned int location =
							EXTRACT_LOCN(ENDIAN_RET32(transactions[i].transdata0));
					unsigned int type =
							EXTRACT_TYPE(ENDIAN_RET32(transactions[i].transdata1));
					unsigned int days =
							EXTRACT_DAYS(ENDIAN_RET32(transactions[i].transdata1));
					unsigned int minutes =
							EXTRACT_MINS(ENDIAN_RET32(transactions[i].transdata1));
					...
					if (counter > 0) {
						...
						emit message(
								QString("Transaction number: %1").arg(counter));
						emit message(
								QString("Date and time: %1 %2:%3").arg(
										epochDate.addDays(days).toString(
												"d MMM yyyy")).arg(hour, 2, 10,
										QChar('0')).arg(mins, 2, 10,
										QChar('0')));
						...
						emit message(
								QString("Transport Network: %1").arg(locText));
						...
						emit message(
								QString("Transaction type: %1").arg(typeText));
					}
				} else {
					emit message(QString("ERROR transmitting APDU:%1").arg(rc));
					emit message(Utilities::getOperationResultName(rc));
			...
			qDebug()
					<< "XXXX NfcWorker::handleGvbEvent - target must be MiFare UL";
	...
	qDebug()
			<< "XXXX NfcWorker::handleGvbEvent - NFC BPS event that we didn't register for: "
			<< code;
}
}
...
uint16_t code = bps_event_get_code(event);
qDebug() << "XXXX NfcWorker::handleReadTagDetailsEvent - processing event code "
		<< code;
...
if (NFC_TAG_READWRITE_EVENT == code) {
	qDebug()
			<< "XXXX NfcWorker::handleReadTagDetailsEvent - Target Read Write Event";
	CHECK(nfc_get_nfc_event(event, &nfcEvent));
	CHECK(nfc_get_target(nfcEvent, &target));
	qDebug() << "XXXX displaying tag info";
...
if (NFC_SNEP_CONNECTION_EVENT == code) {
	...
	QString vCard =
			QString("BEGIN:VCARD\n").append("VERSION:3.0\n").append("N:").append(
					_last_name).append(";").append(_first_name).append("\n").append(
					...
					"EMAIL;TYPE=INTERNET:").append(_email).append("\n").append(
					"END:VCARD");
...
qDebug() << "XXXX NfcWorker::handleEmulateNfcEvent - processing event code: "
		<< code;
...
qDebug() << "XXXX NfcWorker::handleEmulateEchoEvent - processing event code: "
		<< code;
...
result = nfc_get_iso14443_4_emulation_command(target, command,
		NFC_ISO14443_4_COMMAND_BUFFER_LENGTH, &command_length);

if (result != NFC_RESULT_SUCCESS) {
	qDebug()
			<< QString("nfc_get_iso14443_4_emulation_command failed: %d").arg(
					result);
	emit message(
			QString("nfc_get_iso14443_4_emulation_command failed: %d").arg(
					result));
...
QByteArray commandData = QByteArray::fromRawData(
		reinterpret_cast<const char *>(command), command_length);
QString commandAsHex = QString::fromAscii(commandData.toHex());
qDebug() << "XXXX command=" << commandAsHex;

if (Utilities::isSameCharArray(command, FILTER_APDU_0,
		FILTER_APDU_LENGTHS[0])) {
	qDebug() << "XXXX filtering unwanted APDU command";
	result = nfc_send_iso14443_4_emulation_command_response(RESPONSE_OK, 2);
	return;
}
if (Utilities::isSameCharArray(command, FILTER_APDU_1,
		FILTER_APDU_LENGTHS[1])) {
	qDebug() << "XXXX filtering unwanted APDU command";
	result = nfc_send_iso14443_4_emulation_command_response(RESPONSE_OK, 2);
	return;
}
if (Utilities::isSameCharArray(command, FILTER_APDU_2,
		FILTER_APDU_LENGTHS[2])) {
	qDebug() << "XXXX filtering unwanted APDU command";
	result = nfc_send_iso14443_4_emulation_command_response(RESPONSE_OK, 2);
	return;
}
if (Utilities::isSameCharArray(command, FILTER_APDU_3,
		FILTER_APDU_LENGTHS[3])) {
	qDebug() << "XXXX filtering unwanted APDU command";
	result = nfc_send_iso14443_4_emulation_command_response(RESPONSE_OK, 2);
	return;
}
if (Utilities::isSameCharArray(command, FILTER_APDU_4,
		FILTER_APDU_LENGTHS[4])) {
	qDebug() << "XXXX filtering unwanted APDU command";
	result = nfc_send_iso14443_4_emulation_command_response(RESPONSE_OK, 2);
	return;
}
...
emit message(
		QString("Command APDU : %1 len=%2").arg(commandAsHex).arg(
				command_length));
...
response[i] = 0x00;
QByteArray responseData = QByteArray::fromRawData(
		reinterpret_cast<const char *>(response), response_length);
QString responseAsHex = QString::fromAscii(responseData.toHex());

emit message(QString("Response APDU : %1").arg(responseAsHex));
result = nfc_send_iso14443_4_emulation_command_response(response,
		response_length);
if (result != NFC_RESULT_SUCCESS) {
	qDebug()
			<< QString(
					"nfc_send_iso14443_4_emulation_command_response response: %d").arg(
					result);
	emit message(
			QString(
					"nfc_send_iso14443_4_emulation_command_response response: %d").arg(
...
for (int ndefMsgIndex = 0; ndefMsgIndex < ndefMsgCount; ++ndefMsgIndex) {
	...
	for (unsigned int ndefRecordIndex = 0; ndefRecordIndex < ndefRecordCount;
			++ndefRecordIndex) {
		...
		if (strcmp(ndefRecordNdefType, Settings::NfcRtdSmartPoster) == 0) {
			...
			qDebug()
					<< QString("Message(%1)/Record(%2) Language: %3").arg(
							ndefMsgIndex).arg(ndefRecordIndex).arg(found_lang);

			qDebug()
					<< QString("Message(%1)/Record(%2) Title: %3").arg(
							ndefMsgIndex).arg(ndefRecordIndex).arg(utf_title);
			...
			if (uriType == Settings::NfcRtdUriPrefixHttpWww) {
				...
				emit message(
						QString("URI Prefix %1 not implemented").arg(uriType));
			}
			...
			language = QString::fromUtf8(
					payLoadData.mid(1, languageLen).constData(), languageLen);
		...
		qDebug()
				<< QString("Message(%1)/Record(%2) Type: %3").arg(ndefMsgIndex).arg(
						ndefRecordIndex).arg(ndefRecordNdefType);

		qDebug()
				<< QString("Message(%1)/Record(%2) Id: %3").arg(ndefMsgIndex).arg(
						ndefRecordIndex).arg(ndefRecordIdentifier);
...
nfc_ndef_record_t* NfcWorker::makeCustomRecord(QString domain, QString type,
	QString text) {
qDebug()
		<< QString("makeCustomRecord DOMAIN: %1 TYPE: %2 TEXT %3").arg(domain).arg(
				type).arg(text);
nfc_ndef_record_t* record = 0;
...
void NfcWorker::displayTagInformation(nfc_target_t* target,
	bool display_ndef_details) {

struct TagPropertyMap {
	target_property_type_t property;
	const char *name;
} tagPropertyMap[] = { { TAG_PROP_ISO_14443_3_TYPE, "ISO_14443_3 TYPE" }, {
		TAG_PROP_ISO_14443_4_TYPE, "ISO_14443_4 TYPE" }, { TAG_PROP_NDEF_TYPE,
		"NDEF TYPE" }, { TAG_PROP_NDEF_LOCKED, "NDEF LOCKED" }, {
		TAG_PROP_NDEF_LOCKABLE, "NDEF LOCKABLE" }, { TAG_PROP_NDEF_FREE_SPACE,
		"NDEF FREE SPACE" },
		{ TAG_PROP_NDEF_CAPABLE_TYPE, "NDEF CAPABLE TYPE" }, {
				TAG_PROP_NDEF_CAPABLE_TAG_SIZE, "NDEF CAPABLE TAG SIZE" }, {
				TAG_PROP_ISO_15693_3_TYPE, "ISO_15693_3 TYPE" }, {
				TAG_PROP_AFI_SUPPORTED, "AFI SUPPORTED" }, {
				TAG_PROP_AFI_LOCKED, "AFI LOCKED" }, { TAG_PROP_DSFID_SUPPORTED,
				"DSFID SUPPORTED" }, { TAG_PROP_DSFID_LOCKED, "DSFID LOCKED" },
		{ TAG_PROP_AFI, "AFI" }, { TAG_PROP_DSFID, "DSFID" }, {
				TAG_PROP_SECTOR_SIZE, "SECTOR SIZE" }, { TAG_PROP_SECTOR_NUMBER,
				"SECTOR NUMBER" }, { TAG_PROP_KOVIO_TYPE, "KOVIO TYPE" } };

struct TagVariantMap {
	tag_variant_type_t variant;
	const char *name;
} tagVariantMap[] = { { TAGVARIANT_UNKNOWN, "UNKNOWN" }, { TAGVARIANT_JEWEL,
		"JEWEL" }, { TAGVARIANT_TOPAZ, "TOPAZ" }, { TAGVARIANT_TOPAZ_512,
		"TOPAZ 512" }, { TAGVARIANT_MIFARE_UL, "MIFARE UL" }, {
		TAGVARIANT_MIFARE_UL_C, "MIFARE UL C" }, { TAGVARIANT_DESFIRE_D40,
		"DESFIRE D40" }, { TAGVARIANT_DESFIRE_EV1_2K, "DESFIRE EV1 2K" }, {
		TAGVARIANT_DESFIRE_EV1_4K, "DESFIRE EV1 4K" }, {
		TAGVARIANT_DESFIRE_EV1_8K, "DESFIRE EV1 8K" }, { TAGVARIANT_TI_TAGIT,
		"TI TAGIT" }, { TAGVARIANT_ST_LRI_512, "ST LRI 512" }, {
		TAGVARIANT_ST_LRI_2K, "ST LRI 2K" },
		{ TAGVARIANT_NXP_ICODE, "NXP ICODE" }, { TAGVARIANT_KOVIO, "KOVIO" }, {
				TAGVARIANT_MIFARE_1K, "MIFARE 1K" }, { TAGVARIANT_MIFARE_4K,
				"MIFARE 4K" }, { TAGVARIANT_MIFARE_MINI, "MIFARE MINI" }, {
				TAGVARIANT_MY_D_MOVE, "MY D MOVE" }, { TAGVARIANT_MY_D_NFC,
				"MY D NFC" } };
...
for (uint_t i = 0; i < (sizeof(tagPropertyMap) / sizeof(struct TagPropertyMap));
		i++) {
	qDebug() << "XXXX nfc_get_tag_property:" << tagPropertyMap[i].name;
	nfc_result_t rc = nfc_get_tag_property(target, tagPropertyMap[i].property,
	...
	if (rc == NFC_RESULT_SUCCESS) {
		if (strlen(&buffer[0]) > 0) {
			if (strcmp(tagPropertyMap[i].name, "NDEF TYPE") != 0) {
				...
				emit message(
						QString("%1: %2").arg(tagPropertyMap[i].name).arg(
								&buffer[0]));
			} else {
				qDebug() << "XXXX NFC FORUM TYPE" << buffer << endl;
				emit message(
						QString("%1: %2").arg("NFC FORUM TYPE").arg(
								&buffer[0]));
			}
		}
	} else {
		qDebug() << "XXXX nfc_get_tag_property() for" << tagPropertyMap[i].name
				<< "returned" << rc << endl;
	}
}
...
if (rc == NFC_RESULT_SUCCESS) {
	for (uint_t i = 0;
			i < (sizeof(tagVariantMap) / sizeof(struct TagVariantMap)); i++) {
